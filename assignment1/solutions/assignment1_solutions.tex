...
\frac{\partial J}{\partial{\boldsymbol v}_{c}}  &= (\sigma(z_{j}) - \delta^{j}_{o}){\boldsymbol u}_{j}
\end{align}
...
\noindent {\bf Hint}: feel free to use $F({\boldsymbol o}\vert {\boldsymbol v}_{c})$ (where ${\boldsymbol o}$ is the expected word) as a placeholder for $J_{\texttt{softmax-CE}}({\boldsymbol o}, {\boldsymbol v}_{c}, ...)$ or $J_{\texttt{neg-sampling}}({\boldsymbol o}, {\boldsymbol v}_{c}, ...)$ cost functions in this part --- you'll see that this is a useful abstraction for the coding part. That is, your solution may contain terms of the form $\frac{\partial{F_{i}}}{\partial\lambda}$.\\
